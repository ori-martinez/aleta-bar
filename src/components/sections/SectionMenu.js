...
export const SectionMenu = ({ dishes, time, title, onClick }) => {
    ...
    dishes.forEach((item, index) => resultDishes.push(
        <div className="py-4 w-full grid grid-cols-3 gap-x-4 place-items-center md:gap-x-8 lg:gap-x-12 xl:gap-x-16" key={index}>
            ...
            <div className="w-full col-span-2">
                ...
                {// CONDICIONAL
                /* Comprobación de la Existencia de description para el Platillo */
    ...
    return (
        <div className="pt-12 pb-24">
            ...
            {// CONDICIONAL
            /* Comprobación de la Existencia de time para la Sección */
            time !== null && (
                <p className="mx-4 text-sm font-light xs:mx-6 xs:text-base sm:mx-10 sm:text-lg md:mx-16 md:text-xl lg:text-2xl">{ time }</p>
            )}
